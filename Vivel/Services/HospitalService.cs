...
namespace Vivel.Services
{
    public class HospitalService : BaseCRUDService<HospitalDTO, Hospital, HospitalSearchRequest, HospitalUpsertRequest, HospitalUpsertRequest>, IHospitalService
    {
        ...
        {
            var entity = _context.Drives.Include(x => x.Status).Where(x => x.HospitalId == id).AsQueryable();
        ...
        public async Task<HtmlToPdfDocument> DrivesReport(string id, HospitalReportDrivesRequest request)
        {
            ...
            var drives = _context.Drives.Include(x => x.Status).Include(x => x.Donations).AsQueryable();
        ...
        public async Task<HtmlToPdfDocument> LitresByBloodTypeReport(string id, HospitalReportLitresRequest request)
        {
            ...
            var drives = _context.Drives.Include(x => x.Status).AsQueryable();
